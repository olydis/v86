...
export class Ne2k
{
    ...
    private isr = 0;
    private imr = 0; // interrupt mask register
    ...
    private tcnt = 0;
    private tpsr = 0;
    private memory = new Uint8Array(256 * 0x80);
    ...
    private pstart = START_PAGE;
    private pstop = STOP_PAGE;

    private curpg = START_RX_PAGE;
    private boundary = START_RX_PAGE;

    constructor(private readonly cpu: CPU, private readonly bus: BusConnector)
    {
        this.bus.register("net0-receive", (data) =>
        {
            this.receive(data);
        }, this);
        ...
        io.register_read(this.port | E8390_CMD, this, () =>
        {
            dbg_log("Read cmd", LOG_NET);
            return this.cr;
        });

        io.register_write(this.port | E8390_CMD, this, (data_byte) =>
        {
            this.cr = data_byte & ~4;
        ...
        io.register_read(this.port | EN0_COUNTER0, this, () =>
        {
            dbg_log("Read counter0", LOG_NET);
            return 0;
        });

        io.register_read(this.port | EN0_COUNTER1, this, () =>
        {
            dbg_log("Read counter1", LOG_NET);
            return 0;
        });

        io.register_read(this.port | EN0_COUNTER2, this, () =>
        {
            dbg_log("Read counter2", LOG_NET);
            return 0;
        });

        io.register_read(this.port | NE_RESET, this, () =>
        {
        ...
        io.register_write(this.port | NE_RESET, this, (data_byte) =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_write(this.port | EN0_STARTPG, this, (data_byte) =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_write(this.port | EN0_STOPPG, this, (data_byte) =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_read(this.port | EN0_ISR, this, () =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_write(this.port | EN0_ISR, this, (data_byte) =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_write(this.port | EN0_TXCR, this, (data_byte) =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_write(this.port | EN0_DCFG, this, (data_byte) =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_write(this.port | EN0_RCNTLO, this, (data_byte) =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_write(this.port | EN0_RCNTHI, this, (data_byte) =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_write(this.port | EN0_RSARLO, this, (data_byte) =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_write(this.port | EN0_RSARHI, this, (data_byte) =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_write(this.port | EN0_IMR, this, (data_byte) =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_read(this.port | EN0_BOUNDARY, this, () =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_write(this.port | EN0_BOUNDARY, this, (data_byte) =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_read(this.port | EN0_TSR, this, () =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_write(this.port | EN0_TPSR, this, (data_byte) =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_write(this.port | EN0_TCNTLO, this, (data_byte) =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_write(this.port | EN0_TCNTHI, this, (data_byte) =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_read(this.port | EN0_RSR, this, () =>
        {
            var pg = this.get_page();
            if(pg === 0)
            {
        ...
        io.register_write(this.port | EN0_RXCR, this, (data_byte) =>
        {
    ...
    public data_port_write(data_byte)
    {
    ...
    public data_port_read8()
    {
        return this.data_port_read16() & 0xFF;
    };

    public data_port_read16()
    {
        if(this.dcfg & 1)
        {
