...
export class PS2
{
    private enable_mouse_stream = false;
    private use_mouse = false;
    private have_mouse = true;

    private mouse_delta_x = 0;
    private mouse_delta_y = 0;
    private mouse_clicks = 0;

    private have_keyboard = true;
    private enable_keyboard_stream = false;
    private next_is_mouse_command = false;
    private next_read_sample = false;
    private next_read_led = false;
    private next_handle_scan_code_set = false;
    private next_read_rate = false;
    private next_read_resolution = false;

    private kbd_buffer = new ByteQueue(1024);
    private last_port60_byte = 0;
    private sample_rate = 100;
    private resolution = 4;
    private scaling2 = false;
    private last_mouse_packet = -1;
    private mouse_buffer = new ByteQueue(1024);

    private command_register = 1 | 4;
    private read_output_register = false;
    private read_command_register = false;

    constructor(private readonly cpu: CPU, private readonly bus: BusConnector)
    {
        this.bus.register("keyboard-code", (code) =>
        {
            this.kbd_send_code(code);
        }, this);

        this.bus.register("mouse-click", (data) =>
        {
            this.mouse_send_click(data[0], data[1], data[2]);
        }, this);

        this.bus.register("mouse-delta", (data) =>
        {
            this.mouse_send_delta(data[0], data[1]);
        }, this);

        this.bus.register("mouse-wheel", (data) =>
        {
            // TODO: Mouse Wheel
    ...
    public mouse_send_delta(delta_x, delta_y)
    {
    ...
    public mouse_send_click(left, middle, right)
    {
        if(!this.have_mouse || !this.use_mouse)
        {
            return;
    ...
    public send_mouse_packet(dx: number, dy: number): void
    {
        var info_byte =
                (dy < 0 ? 1 : 0) << 5 |
                (dx < 0 ? 1 : 0) << 4 |
    ...
    public port60_write(write_byte)
    {
    ...
    public port64_write(write_byte)
    {
