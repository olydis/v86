...
/// <reference path='lib/9p.d.ts' />
/// <reference path='lib/filsystem.d.ts' />
/// <reference path='lib/jor1k.d.ts' />

type byte = number;

export class VirtIO
{
    private pci_space: byte[];
    ...
    private queue_select = 0;
    private device_status = 0;
    private isr = 0;

    // these should be stored per queue if there is more than one queue
    private last_idx = 0;
    private queue_size = 32;
    private queue_address = 0;

    private device: Virtio9p;
        ...
        filesystem: FS)
    {
        ...
        io.register_write(0xA804, this, undefined, undefined, (data) =>
        {
            // write guest features
            dbg_log("Guest feature selection: " + h(data, 8), LOG_VIRTIO);
        });

        io.register_write(0xA80E, this, undefined, (data) =>
        {
            // rw queue select
        ...
        io.register_read(0xA80C, this, undefined, () =>
        {
            // read queue size
        ...
        io.register_read(0xA808, this, undefined, undefined, () =>
        {
            // rw queue address
        ...
        io.register_write(0xA808, this, undefined, undefined, (data) =>
        {
            // rw queue address
        ...
        io.register_write(0xA812, this, (data) =>
        {
        ...
        io.register_write(0xA810, this, undefined, (data) =>
        {
        ...
        for(var port = 0; port < 128; port++)
        {
            io.register_read(0xA814 + port, this, () =>
            {
    ...
    public set_state(state: any): void
    {
        this.irq = state[0];
    ...
    public handle_descriptor(idx: any): void
    {
        var next = idx;
        ...
        this.device.ReceiveRequest(infos, () =>
        {
            // return one byte
    ...
    public device_reply(queueidx: any, infos: any): void
    {
        if(infos.next === -1)
        {
