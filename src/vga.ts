...
export class VGAScreen
{
    ...
    private vga_memory_size: number;
    private cursor_address: number;
    private cursor_scanline_start: number;
    private cursor_scanline_end: number;
    private max_cols: number; // Number of columns in text mode
    private max_rows: number; // Number of rows in text mode
    private screen_width: number; // Width in pixels in graphical mode
    private screen_height: number; // Height in pixels in graphical mode
    private start_address: number; // video memory start address
    private crtc: Uint8Array;
    private previous_start_address: number;
    private graphical_mode_is_linear: boolean;
    private graphical_mode: boolean;
    private vga256_palette: Int32Array; // VGA palette containing 256 colors for video mode 13 etc. Needs to be initialised by the BIOS

    // VGA latches
    private latch0: number;
    private latch1: number;
    private latch2: number;
    private latch3: number;

    private svga_width: number;
    private svga_height: number;
    private text_mode_width: number;
    private svga_enabled: boolean;
    private svga_bpp: number;
    private svga_bank_offset: number;
    private svga_offset: number; // The video buffer offset created by VBE_DISPI_INDEX_Y_OFFSET (In bytes)

    private pci_space: number[];
    private pci_id: number;
    private pci_bars: { size: number }[];
    private name: string;
    ...
    private index_crtc: number;

    private offset_register: number;

    // index for setting colors through port 3C9h
    private dac_color_index_write: number;
    private dac_color_index_read: number;

    private attribute_controller_index: number;

    private dac_map: Uint8Array;

    private sequencer_index: number;

    // bitmap of planes 0-3
    private plane_write_bm: number;
    private sequencer_memory_mode: number;
    private graphics_index: number;

    private plane_read: number; // value 0-3, which plane to read
    private planar_mode: number;
    private planar_rotate_reg: number;
    private planar_bitmap: number;

    private max_scan_line: number;

    private miscellaneous_output_register: number;
    private port_3DA_value: number;

    // Bochs VBE Extensions
    // http://wiki.osdev.org/Bochs_VBE_Extensions
    private dispi_index: number;
    private dispi_enable_value: number;

    private svga_memory: Uint8Array;

    private diff_addr_min: number;
    private diff_addr_max: number;

    private dest_buffer: any;

    private svga_memory16: Uint16Array;
    private svga_memory32: Int32Array;
    private vga_memory: Uint8Array;
    private plane0: Uint8Array;
    private plane1: Uint8Array;
    private plane2: Uint8Array;
    private plane3: Uint8Array;


    constructor(cpu: CPU, bus: BusConnector, vga_memory_size: number)
    {
        this.bus = bus;
        ...
        bus.register("screen-tell-buffer", (data) =>
        {
            this.dest_buffer = data[0];
        }, this);

        bus.register("screen-fill-buffer", () => 
        {
    ...
    public vga_memory_read(addr): any
    {
        //dbg_log("vga memory read: " + h(addr, 5), LOG_VGA);
        addr -= 0xA0000;
    ...
    public vga_memory_write(addr, value): void
    {
        //dbg_log("vga memory write: " + h(addr, 5), LOG_VGA);
        addr -= 0xA0000;
    ...
    public vga_memory_write_graphical_linear(addr, value): void
    {
        //dbg_log("vga memory write: " + h(addr, 5) + " " + h(value, 2), LOG_VGA);
        addr |= this.svga_bank_offset;
    ...
    public vga_memory_write_graphical_planar(addr, value): void
    {
        if(addr > 0xFFFF)
        {
            return;
    ...
    public vga_memory_write_text_mode(addr, value): void
    {
        if(addr < 0x18000)
        {
            return;
    ...
    public svga_memory_read8(addr): any
    {
        return this.svga_memory[addr & 0xFFFFFFF];
    }

    public svga_memory_read32(addr): any
    {
        addr &= 0xFFFFFFF;
    ...
    public svga_memory_write8(addr, value): void
    {
        addr &= 0xFFFFFFF;
        this.svga_memory[addr] = value;
    ...
    public svga_memory_write32(addr, value): void
    {
        addr &= 0xFFFFFFF;
    ...
    public set_size_text(cols_count: number, rows_count: number): void
    {
        //console.log("text", cols_count, rows_count);
        this.max_cols = cols_count;
        this.max_rows = rows_count;

        if(cols_count === 40 && rows_count === 25)
        {
            cols_count = 80;
        }
    ...
    public set_size_graphical(width, height, bpp): void
    {
    ...
    public port3C0_write(value: any): void
    {
    ...
    public port3C2_write(value: any): void
    {
    ...
    public port3C4_write(value: any): void
    {
        this.sequencer_index = value;
    }

    public port3C4_read(): any
    {
        return this.sequencer_index;
    }

    public port3C5_write(value: any): void
    {
    ...
    public port3C7_write(index: number): void
    {
    ...
    public port3C8_write(index: number): void
    {
        this.dac_color_index_write = index * 3;
    }

    public port3C9_write(color_byte: number): void
    {
    ...
    public port3CC_read(): any
    {
        return this.miscellaneous_output_register;
    }

    public port3CE_write(value: any): void
    {
        this.graphics_index = value;
    }

    public port3CE_read(): any
    {
        return this.graphics_index;
    }

    public port3CF_write(value: any): void
    {
    ...
    public port3D4_write(register: any): void
    {
        this.index_crtc = register;
    }

    public port3D5_write(value: number): void
    {
    ...
    public switch_video_mode(mar: number): void
    {
    ...
    public svga_bytes_per_line(): number
    {
    ...
    public port1CE_write(value: any): void
    {
        this.dispi_index = value;
    }

    public port1CF_write(value: any): void
    {
    ...
    public screen_fill_buffer(): void
    {
        if(!this.graphical_mode)
        {
            // text mode
